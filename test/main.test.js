...
QUnit.test('TEST  Perimeter Of Triangle', assert => {
  assert.equal(perim(0, 0,0), 0, 'Zero')
  assert.equal(perim(10, 20,30), 60, 'Integers')
  assert.equal(perim(300, 200,500), 1000, 'long Integers')
  assert.equal(perim(-1, -3,-4), "Error", 'Negative values')
  assert.equal(perim(200, -1,-89), "Error", 'Negative values')
})
